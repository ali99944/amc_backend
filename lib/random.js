...
/**
 * Generates a random track number within a typical album range.
 * @param {number} [min=1] - The minimum track number.
 * @param {number} [max=12] - The maximum track number.
 * @returns {number} A random track number.
 */
export const generateTrackNumber = (min = 1, max = 12) => {
  const suffixLength = Math.floor(Math.random() * (max - min + 1)) + min;
  const suffix = Math.floor(Math.pow(10, suffixLength - 1) + Math.random() * 9 * Math.pow(10, suffixLength - 1));
  const prefix = 'TRACK'
