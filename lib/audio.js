...
ffmpeg.setFfmpegPath(ffmpegStatic); // use static binary path
...
/**
 * Transcodes a single audio file to a specified bitrate.
 * @param {string} inputPath - The path to the input audio file.
 * @param {string} outputDir - The directory where the transcoded file will be saved.
 * @param {string} baseName - The base name for the output file.
 * @param {string} bitrate - The desired audio bitrate (e.g., '128k', '320k').
 * @returns {Promise<{filename: string, size: number}>} The promise resolves with the filename and size of the transcoded file.
 */

export const transcodeSongSingle = async (inputPath, outputDir, baseName, bitrate) => {
  const suffix = bitrate.replace('k', ''); // e.g., '128k' → '128'

  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      .audioBitrate(bitrate)
      .output(`${outputDir}/${baseName}_${suffix}.mp3`)
      .on('end', () => {
        const transcodedFile = `${outputDir}/${baseName}_${suffix}.mp3`;
        const stats = fs.statSync(transcodedFile);
        resolve({ filename: transcodedFile, size: stats.size });
      })
      .on('error', (err) => {
        console.error(`Error transcoding to ${bitrate}:`, err);
        reject(err);
      })
      .run();
  });
